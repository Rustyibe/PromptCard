    ...
    appState.categories = [
      {
        ...
        prompts: [
          ...
          { id: '1-2', content: '为具有以下特征的主角开发详细的角色档案: [TRAITS]' }
        ]
      },
      {
        id: '2',
...
function renderCategories() {
  ...
  const categoriesToRender = appState.filteredCategories.length > 0 
  ...
  categoriesToRender.forEach(category => {
    ...
    if (appState.currentCategory && category.id === appState.currentCategory.id) {
...
function renderPrompts() {
  ...
  if (appState.searchQuery && appState.filteredCategories.length > 0) {
    ...
    appState.filteredCategories.forEach(category => {
      ...
      category.prompts.forEach((prompt, index) => {
        ...
        contentDiv.textContent = prompt.content.substring(0, 120) + (prompt.content.length > 120 ? '...' : '');
  ...
  if (!appState.currentCategory) {
    promptsList.innerHTML = '<div class="no-selection">选择一个主题以查看提示词</div>';
    return;
  }
...
function handleDrop(e) {
  ...
  if (appState.dragItem !== this) {
    ...
    const dragIndex = Array.from(promptsList.children).indexOf(appState.dragItem);
    const dropIndex = Array.from(promptsList.children).indexOf(this);
    
    if (dragIndex < dropIndex) {
      // Moving down
      appState.currentCategory.prompts.splice(dropIndex + 1, 0, 
        appState.currentCategory.prompts.splice(dragIndex, 1)[0]);
    } else {
      // Moving up
      appState.currentCategory.prompts.splice(dropIndex, 0, 
        appState.currentCategory.prompts.splice(dragIndex, 1)[0]);
    }
...
async function deleteCategory(categoryId) {
  const categoryIndex = appState.categories.findIndex(category => category.id === categoryId);
  if (categoryIndex === -1) {
    return;
  }
  ...
  const isCurrentCategory = appState.currentCategory && appState.currentCategory.id === categoryId;
...
async function editCategory(categoryId) {
  ...
  const newCategoryName = prompt('请输入新的主题名称:', category.name);
  if (newCategoryName && newCategoryName !== category.name) {
    ...
    if (appState.currentCategory && appState.currentCategory.id === categoryId) {
      appState.currentCategory.name = newCategoryName;
      selectedCategoryTitle.textContent = newCategoryName;
      if (appState.view === 'prompts') {
        pageTitle.textContent = newCategoryName;
      }
    }
  }
}
...
async function editPrompt(categoryId, promptId, currentContent) {
  ...
  const promptIndex = category.prompts.findIndex(item => item.id === promptId);
  if (promptIndex === -1) {
    return;
  }

  const newPromptContent = prompt('编辑提示词内容:', currentContent);
  if (newPromptContent !== null && newPromptContent !== currentContent) {
    category.prompts[promptIndex].content = newPromptContent;
    
    // 如果当前选中的是该类别，更新当前类别的引用
    if (appState.currentCategory && appState.currentCategory.id === categoryId) {
      appState.currentCategory.prompts[promptIndex].content = newPromptContent;
    }
...
async function addNewCategory() {
  const categoryName = prompt('请输入主题名称:');
  if (categoryName) {
    const newCategory = {
...
async function addNewPrompt() {
  ...
  const promptContent = prompt('请输入提示词内容:');
  if (promptContent) {
    const newPrompt = {
...
async function deletePrompt(categoryId, promptId) {
  ...
  if (appState.currentCategory && appState.currentCategory.id === categoryId) {
    appState.currentCategory = { ...appState.currentCategory, prompts: [...category.prompts] };
    const currentCategoryIndex = appState.categories.findIndex(item => item.id === categoryId);
  ...
  appState.categories.forEach(category => {
    ...
    if (categoryMatches || matchingPrompts.length > 0) {
      ...
      appState.filteredCategories.push({
        ...category,
        prompts: matchingPrompts.length > 0 ? matchingPrompts : category.prompts
      });
    }
  });
  ...
  input.onchange = async (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const confirmImport = confirm(`确定要导入 ${importedData.categories.length} 个主题吗？\n注意：这将会覆盖当前的所有数据。`);
...
async function exportData() {
  try {
    ...
    a.href = url;
    a.download = `ai-prompts-export-${new Date().toISOString().slice(0, 10)}.json`;
